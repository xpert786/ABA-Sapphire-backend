...
class SessionAdmin(admin.ModelAdmin):
    list_display = ('id', 'client', 'staff', 'session_date', 'start_time', 'end_time', 'created_at')
    list_filter = ('session_date', 'staff', 'client', 'created_at')
    search_fields = ('client__name', 'client__username', 'staff__name', 'staff__username', 'session_notes')
    readonly_fields = ('created_at',)
    list_per_page = 25
    
    fieldsets = (
        ('Session Details', {
            'fields': ('client', 'staff', 'session_date', 'start_time', 'end_time')
        }),
        ('Session Information', {
    ...
    def get_queryset(self, request):
        return super().get_queryset(request).select_related('client', 'staff')
